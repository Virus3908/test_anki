...
extension KanjiDataLoader {
    ...
    static func loadAvailableCards(deck: KanjiDeck) async -> [KanjiCard] {
        ...
        let cachedCards = (try? await KanjiDeckCacheRepository.shared.loadCards(for: .all)) ?? []
        ...
        if let cached = try? await KanjiDeckCacheRepository.shared.loadCards(for: deck), !cached.isEmpty {
